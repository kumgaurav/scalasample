...
object SampleEventCSVToTable extends App {
    ...
    val columns = Seq("pubsub_attribute_metadata","retry_attempt_number","record_ingestion_time","filetype","sha256","mid","source","type","analysis","platform","update_date","sha1","size","family","create_date","md5","status","action","metadata")
    import spark.implicits._
    val finaldf = csvdf.select(columns.map(c => col(c)): _*)
    finaldf.show(100)
    finaldf.write.mode(SaveMode.Overwrite).parquet("gs://itd-aia-de/temp/virusdb/sample_event/")
}
